                                            ...
                                            <tr>
                                                ...
                                                <td width="50%">
                                                    <table width="100%" align="left" cellpadding="0" cellspacing="0">
                                                        <tr>
                                                            <td width="50%" valign="bottom" style="line-height:12px; font-size:9px;">Position:{{$positionData->position_name}}<br><?php if($positionData->color_stitch_count > 0) {?>Colors: {{$positionData->color_stitch_count}}<br><?php }?>{{$positionData->placement_type_name}} @ {{$positionData->placement_type_charge}}<br><?php if($positionData->press_setup_qnty > 0){?>Press Setup @ {{$positionData->press_setup_qnty * $price_grid_data[0]->press_setup}} <br><?php }?><?php if($positionData->screen_fees_qnty > 0){?>Screen Fee @ {{$positionData->screen_fees_qnty * $price_grid_data[0]->screen_fees }}<br><?php }?><?php if($positionData->speciality_qnty > 0) {?>Speciality Ink @ {{$positionData->speciality_qnty * $price_grid_data[0]->specialty}}<?php }?><br><?php if($positionData->foil_qnty > 0) {?>Foil @ {{$positionData->foil_qnty * $price_grid_data[0]->foil}}<?php }?><br><?php if($positionData->number_on_dark_qnty > 0) {?>Number On Dark @ {{$positionData->number_on_dark_qnty * $price_grid_data[0]->number_on_dark}}<?php }?><br><?php if($positionData->number_on_light_qnty > 0) {?>Number On Light @ {{$positionData->number_on_light_qnty * $price_grid_data[0]->number_on_light}}<?php }?><br><?php if($positionData->oversize_screens_qnty > 0) {?>Oversize Screen @ {{$positionData->oversize_screens_qnty * $price_grid_data[0]->over_size_screens}}<?php }?><br><?php if($positionData->ink_charge_qnty > 0) {?>Ink Charge @ {{$positionData->ink_charge_qnty * $price_grid_data[0]->ink_changes}}<?php }?><br><?php if($positionData->discharge_qnty > 0) {?>Discharge @ {{$positionData->discharge_qnty * $price_grid_data[0]->discharge}}<?php }?><br>
                                                            </td>
