                            ...
                            <tr>
                              ...
                              <td width="95%" valign="middle" style="border:1px solid #666; font-size:10px; height: 98px;">
                                <table width="100%" align="left" border="0" cellspacing="0" cellpadding="0">
                                  ...
                                  <tr>
                                    <td>{{$order_data[0]->client_company}}<br/><?php if(!empty($client_data)){?>{{$client_data[0]->first_name}} {{$client_data[0]->last_name}}<?php } ?><br>@foreach ($addresses['result'] as $address)<?php if($address->address_billing == '1') {?>{{$address->address}}<br>{{$address->street}}<br>{{$address->city}}<br>{{$address->state_name}}<br>{{$address->postal_code}}<?php } ?>@endforeach
                                    </td>
                                  </tr>
                                </table>
                              </td>
          ...
          @foreach($all_design as $index => $design)
                      ...
                      <table width="100%" align="center" border="0" cellspacing="0" cellpadding="2" style="font-family: arial; font-size:10px; border-collapse:collapse;">
                          ...
                          <tr style="background-color:<?php echo $color_bg?>;">
                              ...
                              <td height="20" style="border:1px solid #000; text-align:left; font-size:9px; line-height:20px;"><?php $lenColor = count($product->sizeData);  $color_count = 1;foreach ($product->sizeData as $color_all_name => $colorDataNew) { ?>{{$color_all_name}} <?php if ($color_count != $lenColor) { ?>/ <?php }?><?php $color_count++;}?></td>
                              ...
                              {
                               foreach ($colorData as $size){?>{{$size->size}}:(<?php echo number_format($size->qnty); ?>)&nbsp;
                                <?php  }?><?php if ($sizeAll_count != $lenSize){?>/ <?php }?><?php $sizeAll_count++;}?>
                              </td>
