...
.controller('TableController', ["$scope", "$attrs", "$filter", "tableConfig", "tastyUtil", function($scope, $attrs, $filter, tableConfig, tastyUtil) {
  ...
  if (angular.isDefined($attrs.bindResource)) {
    ...
    } else if (!$scope.resource.data.header && !$scope.resource.data.rows) {
      throw new Error('Angular tastyTable directive: the bind-resource ('+
  ...
  vm.initTable = function (keyDirective) {
    ...
    if (vm.start) {
      if ($scope.clientSide) {
        $scope.params.sortBy = $scope.resource.data.sortBy || $scope.init.sortBy;
        $scope.params.sortOrder = $scope.resource.data.sortOrder || $scope.init.sortOrder;
        $scope.params.page = $scope.init.page;
        if ($scope.resource.data.pagination) {
          $scope.params.page = $scope.resource.data.pagination.page || $scope.init.page;
        }
        if (initNow) {
  ...
  setDirectivesValues = function (resource) {
    if (!angular.isObject(resource)) {
      throw new Error('Angular tastyTable directive: the resource response '+
                      'is not an object');
    } else if (!resource.data.header && !resource.data.rows) {
    ...
    if (resource.data.header.length && Object.keys(resource.data.header[0]).length === 1) {
      resource.header = resource.data.header.map(function (header) {
    ...
    $scope.header = {
      'columns': resource.data.header,
      'sortBy': $scope.params.sortBy,
      'sortOrder': $scope.params.sortOrder
    };
    
    if (!$scope.clientSide) {
      $scope.header.sortBy = $scope.header.sortBy || resource.data.sortBy;
      $scope.header.sortOrder = $scope.header.sortOrder || resource.data.sortOrder;
    }
    $scope.rows = resource.data.rows;
    if ($scope.paginationDirective) {
      ...
      if (resource.data.pagination) {
        if (resource.data.pagination.count) {
          $scope.pagination.count = resource.data.pagination.count;
        }
        if (resource.data.pagination.page) {
          $scope.pagination.page = resource.data.pagination.page;
        }
        if (resource.data.pagination.size) {
          $scope.pagination.size = resource.data.pagination.size;
        }
      }
  ...
  buildUrl = function(params, filters) {
    ...
    if ($scope.theadDirective) {
      urlQuery.sorts = tastyUtil.setProperty(urlQuery.sorts, params, 'sortBy');
      urlQuery.sorts = tastyUtil.setProperty(urlQuery.sorts, params, 'sortOrder');
    }
    if ($scope.paginationDirective) {
      urlQuery.page = tastyUtil.setProperty(urlQuery.page, params, 'page');
      urlQuery.page = tastyUtil.setProperty(urlQuery.page, params, 'count');
    }
    if ($attrs.bindFilters) {
      urlQuery.filter = tastyUtil.joinObjects(urlQuery.filter, filters, listKeyNotJoin);
    }
    return urlQuery;
  ...
  updateClientSideResource = function (updateFrom) {
    if ($scope.params.sortBy) {
      $scope.resource.data.sortBy = $scope.params.sortBy;
    }
    if ($scope.params.sortOrder) {
      $scope.resource.data.sortOrder = $scope.params.sortOrder;
    }
    if ($scope.params.page && $scope.params.count) {
      $scope.resource.data.pagination = $scope.pagination;
      $scope.resource.data.pagination.page = $scope.params.page;
      $scope.resource.data.pagination.count = $scope.params.count;
    }
  ...
  if ($scope.resource) {
    var watchResource = function (newValue, oldValue){
      if (newValue !== oldValue) {
        $scope.params.sortBy = $scope.resource.data.sortBy || $scope.params.sortBy;
        $scope.params.sortOrder = $scope.resource.data.sortOrder || $scope.params.sortOrder;
        $scope.$evalAsync(updateClientSideResource('resource'));
        if (!$scope.resource.data.reload) {
          $scope.resource.data.reload = function reloadResource () {
            $scope.$evalAsync(updateClientSideResource('resource'));
          };
        }
      }
    ...
    if ($scope.watchResource === 'reference') {
      ...
      $scope.$watchCollection('resource.data.header', watchResource);
      $scope.$watchCollection('resource.data.rows', watchResource);
      $scope.$watchGroup(['resource.data.sortBy', 
        'resource.data.sortOrder', 
        'resource.data.pagination.count',
        'resource.data.pagination.page',
        'resource.data.pagination.pages',
        'resource.data.pagination.size'], watchResource);
    } else if ($scope.watchResource === 'equality') {
      $scope.$watch('resource.data.header', watchResource, true);
      $scope.$watch('resource.data.rows', watchResource, true);
      $scope.$watch('resource.data.sortBy', watchResource, true);
      $scope.$watch('resource.data.sortOrder', watchResource, true);
      $scope.$watch('resource.data.pagination.count', watchResource, true);
      $scope.$watch('resource.data.pagination.page', watchResource, true);
      $scope.$watch('resource.data.pagination.pages', watchResource, true);
      $scope.$watch('resource.data.pagination.size', watchResource, true);
    }
  }
}])
  ...
  return {
    ...
    link: function postLink(scope, element, attrs, tastyTable) {
      ...
      scope.setColumns = function () {
        ...
        scope.columns = [];
        if (scope.header.sortOrder === 'desc' && 
            scope.header.sortBy &&
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('template/table/head.html',
    /*'<tr role="row">\n' +
    '  <th class="secondary-text sorting" ng-click="sortBy(column)" ng-repeat="column in columns track by $index" \n' +
    '  ng-class="classToShow(column)"\n' +
    '  ng-style="::column.style" ng-click="sortBy(column)">\n' +
    '<div class="table-header"> \n' +
      '<span class="column-title" ng-bind="::column.name"></span> \n' +
    '</div> \n' +
    '  </th> \n' +
    '</tr>');*/
    '<tr role="row">\n' +
    '  <th class="secondary-text cursor-p" ng-repeat="column in columns track by $index" \n' +
    '  ng-class="classToShow(column)"\n' +
    '  ng-style="::column.style" ng-click="sortBy(column)">\n' +
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('template/table/pagination.html',
    ...
    '  <p><b>Total  <span ng-bind="pagination.size"></span>  record(s)</b> \n' +
    '  </p></div>\n' +
    ...
    '          <span class="sr-only" ng-if="classNumPage(numPage)"></span>\n' +
    '        </a>\n' +
    '      </li>\n' +
    ...
    '    <p><b>Page <span ng-bind="pagination.page"></span> \n' +
